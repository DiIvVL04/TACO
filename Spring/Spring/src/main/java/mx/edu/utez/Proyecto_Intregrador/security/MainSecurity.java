...
public class MainSecurity {
    ...
    private final String[] WHITE_LIST = {
            //poner todos los endpoints que quiero que se muestren; los ** significan que admitan t0do
            "/api/Proyecto_Integrador/auth/**", //signin
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception{
        http.cors(Customizer.withDefaults()).csrf(AbstractHttpConfigurer::disable).authorizeHttpRequests(req ->
                        req.requestMatchers(WHITE_LIST).permitAll()
                                ...
                                .requestMatchers(COCINA).hasAnyAuthority("Cocina")
                                .requestMatchers(MESERO).hasAnyAuthority("Mesero")
